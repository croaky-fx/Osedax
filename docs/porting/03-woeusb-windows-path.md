# WoeUSB → Osedax Windows-media path
...
7. `parted mklabel msdos` (GPT UNSUPPORTED in WoeUSB — Osedax SHOULD add GPT) →
8. data partition at **4MiB** start (FAT: `4MiB 100%`; NTFS: `4MiB -- -2049s`) →
9. `blockdev --rereadpt` + **sleep 3** (Osedax: udev settle instead) →
...
**Osedax default: B** (no wimlib dep, verbatim copy, UEFI:NTFS restores bootability).
...
Osedax: pure-Rust WIM reader preferred (drops 7z dep).
...
- ng warns-but-doesn't-abort on non-root → Osedax check capabilities up front.
